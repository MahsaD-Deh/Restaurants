...
namespace Restaurants.Application.Commands.Restaurant.Create.Tests
{
    public class CreateRestaurantCommandHandlerTests
    {
        ...
        public async Task HandleTest()
        {
            ...
            var restaurant = new Domain.Model.Restaurant();

            mapperMock.Setup(m => m.Map<Domain.Model.Restaurant>(command)).Returns(restaurant);
            
            var restaurantRepositoryMock = new Mock<IRestaurantsRepository>();
            restaurantRepositoryMock
                .Setup(repo => repo.Create(It.IsAny<Domain.Model.Restaurant>()))
                .ReturnsAsync(1);
